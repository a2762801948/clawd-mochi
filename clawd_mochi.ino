 ...
 *   🦀 Cute expressions edition — heart/sleep/star/surprise/blush
 *
 *   Wiring:
...
uint16_t C_PINK, C_RED, C_YELLOW, C_BLUE, C_LAVENDER;
...
#define VIEW_CODE          2
#define VIEW_DRAW          3
#define VIEW_EYES_HEART    4
#define VIEW_EYES_SLEEPY   5
#define VIEW_EYES_STAR     6
#define VIEW_EYES_SURPRISE 7
#define VIEW_EYES_BLUSH    8
...
void initColours() {
  ...
  C_PINK     = tft.color565(255, 105, 140);
  C_RED      = tft.color565(220,  40,  40);
  C_YELLOW   = tft.color565(255, 220,  50);
  C_BLUE     = tft.color565(80, 160, 255);
  C_LAVENDER = tft.color565(200, 160, 255);
...
void drawHeartEyes(bool small = false) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0) + EYE_W / 2;
  const int16_t rx = eyeRX(0) + EYE_W / 2;
  const int16_t cy = eyeCY();
  const int16_t sz = small ? 20 : 32;
  drawHeart(lx, cy, sz, C_PINK);
  drawHeart(rx, cy, sz, C_PINK);
}

void drawSleepyEyes(int16_t droop = 0) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0), rx = eyeRX(0);
  const int16_t cy = eyeCY() + droop;
  tft.fillRect(lx + 2, cy - 3, EYE_W - 4, 6, C_BLACK);
  tft.fillRect(rx + 2, cy - 3, EYE_W - 4, 6, C_BLACK);
  tft.setTextColor(C_LAVENDER); tft.setTextSize(2);
  tft.setCursor(rx + EYE_W + 4, cy - 28); tft.print("Z");
  tft.setTextSize(1);
  tft.setCursor(rx + EYE_W + 14, cy - 38); tft.print("z");
}

void drawStar(int16_t cx, int16_t cy, int16_t r, uint16_t color) {
  tft.fillCircle(cx, cy, r/3, color);
  tft.drawLine(cx, cy - r, cx, cy + r, color);
  tft.drawLine(cx - 1, cy - r, cx - 1, cy + r, color);
  tft.drawLine(cx - r, cy, cx + r, cy, color);
  tft.drawLine(cx - r, cy - 1, cx + r, cy - 1, color);
  int16_t d = r * 0.7;
  tft.drawLine(cx - d, cy - d, cx + d, cy + d, color);
  tft.drawLine(cx + d, cy - d, cx - d, cy + d, color);
  tft.drawLine(cx - d + 1, cy - d, cx + d + 1, cy + d, color);
  tft.drawLine(cx + d - 1, cy - d, cx - d - 1, cy + d, color);
}

void drawStarEyes(bool small = false) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0) + EYE_W / 2;
  const int16_t rx = eyeRX(0) + EYE_W / 2;
  const int16_t cy = eyeCY();
  const int16_t r = small ? 10 : 16;
  drawStar(lx, cy, r, C_YELLOW);
  drawStar(rx, cy, r, C_YELLOW);
}

void drawSurprisedEyes(int16_t r = 22) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0) + EYE_W / 2;
  const int16_t rx = eyeRX(0) + EYE_W / 2;
  const int16_t cy = eyeCY();
  tft.fillCircle(lx, cy, r, C_BLACK);
  tft.fillCircle(rx, cy, r, C_BLACK);
  tft.fillCircle(lx, cy, r - 6, C_WHITE);
  tft.fillCircle(rx, cy, r - 6, C_WHITE);
  tft.fillCircle(lx, cy, r / 3, C_BLACK);
  tft.fillCircle(rx, cy, r / 3, C_BLACK);
  tft.fillCircle(lx - r/4, cy - r/4, 2, C_WHITE);
  tft.fillCircle(rx - r/4, cy - r/4, 2, C_WHITE);
}

void drawBlushEyes(bool blinkPhase = false) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0) + EYE_W / 2;
  const int16_t rx = eyeRX(0) + EYE_W / 2;
  const int16_t cy = eyeCY();
  if (!blinkPhase) {
    for (int8_t t = -4; t <= 4; t++) {
      int16_t dy = -(EYE_W/2 - abs(t) * 2);
      tft.fillRect(lx + t * 3 - 1, cy + dy - 1, 3, 4, C_BLACK);
      tft.fillRect(rx + t * 3 - 1, cy + dy - 1, 3, 4, C_BLACK);
    }
  } else {
    tft.fillRect(lx - 10, cy - 2, 20, 3, C_BLACK);
    tft.fillRect(rx - 10, cy - 2, 20, 3, C_BLACK);
  }
  tft.fillCircle(lx - 28, cy + 18, 10, C_PINK);
  tft.fillCircle(rx + 28, cy + 18, 10, C_PINK);
  tft.fillCircle(lx - 28, cy + 18, 5, animBgColor);
  tft.fillCircle(rx + 28, cy + 18, 5, animBgColor);
  tft.fillCircle(lx - 28, cy + 18, 4, tft.color565(255, 150, 170));
  tft.fillCircle(rx + 28, cy + 18, 4, tft.color565(255, 150, 170));
}
...
// ═════════════════════════════════════════════════════════════
//  ANIMATIONS — Cute expressions
// ═════════════════════════════════════════════════════════════

void animHeartEyes() {
  busy = true;
  for (uint8_t i = 0; i < 3; i++) {
    drawHeartEyes(false); delay(speedMs(200));
    drawHeartEyes(true);  delay(speedMs(150));
  }
  drawHeartEyes(false);
  busy = false;
}
...
void animSleepyEyes() {
  busy = true;
  for (uint8_t i = 0; i < 3; i++) {
    drawSleepyEyes(0);   delay(speedMs(400));
    drawSleepyEyes(8);   delay(speedMs(300));
    drawSleepyEyes(14);  delay(speedMs(500));
    drawSleepyEyes(8);   delay(speedMs(300));
  }
  drawSleepyEyes(14);
  busy = false;
...
void animStarEyes() {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    drawStarEyes(false); delay(speedMs(180));
    drawStarEyes(true);  delay(speedMs(120));
  }
  drawStarEyes(false);
  busy = false;
}

void animSurprisedEyes() {
  busy = true;
  drawSurprisedEyes(16); delay(speedMs(120));
  drawSurprisedEyes(20); delay(speedMs(100));
  drawSurprisedEyes(24); delay(speedMs(200));
  drawSurprisedEyes(22); delay(speedMs(100));
  drawNormalEyes(0, true); delay(speedMs(100));
  drawSurprisedEyes(22);
  busy = false;
}
...
void animBlushEyes() {
  busy = true;
  for (uint8_t i = 0; i < 3; i++) {
    drawBlushEyes(false); delay(speedMs(250));
    drawBlushEyes(true);  delay(speedMs(120));
  }
  drawBlushEyes(false);
  busy = false;
}
...
.vbtn[data-v="4"].active{border-color:#ff6b9a;background:#281018}
.vbtn[data-v="5"].active{border-color:#a0a0ff;background:#101028}
.vbtn[data-v="6"].active{border-color:#ffd700;background:#282010}
.vbtn[data-v="7"].active{border-color:#60c0ff;background:#0c1828}
.vbtn[data-v="8"].active{border-color:#ff8ab0;background:#281020}
...
<div class="vgrid">
  ...
  <button class="vbtn" data-v="1" onclick="setView(1)">
    <span class="ic">&gt; &lt;</span>
    <span class="nm">Squish</span>
    <span class="ht">happy squint</span>
  </button>
  <button class="vbtn" data-v="4" onclick="setView(4)">
    <span class="ic" style="color:#ff6b9a">&#9829; &#9829;</span>
    <span class="nm">Heart</span>
    <span class="ht">love beating</span>
  </button>
  <button class="vbtn" data-v="5" onclick="setView(5)">
    <span class="ic" style="color:#a0a0ff">&#8764; &#8764;</span>
    <span class="nm">Sleepy</span>
    <span class="ht">drowsy nod</span>
  </button>
  <button class="vbtn" data-v="6" onclick="setView(6)">
    <span class="ic" style="color:#ffd700">&#10022; &#10022;</span>
    <span class="nm">Star</span>
    <span class="ht">sparkle twinkle</span>
  </button>
  <button class="vbtn" data-v="7" onclick="setView(7)">
    <span class="ic" style="color:#60c0ff">O O</span>
    <span class="nm">Surprised</span>
    <span class="ht">wide eyes pop</span>
  </button>
  <button class="vbtn" data-v="8" onclick="setView(8)">
    <span class="ic" style="color:#ff8ab0">^ ^</span>
    <span class="nm">Blush</span>
    <span class="ht">shy + cheeks</span>
  </button>
  <button class="vbtn" data-v="2" onclick="setView(2)">
    <span class="ic">{ }</span>
    <span class="nm">Claude Code</span>
    <span class="ht">terminal</span>
  </button>
</div>
<div class="sec">// canvas</div>
<div class="ctrl">
  <button class="vbtn" data-v="3" onclick="toggleCanvas()" style="flex:1">
    <span class="ic">&#11035;</span>
...
<script>
let activeView=0,termOpen=false,canvasOpen=false,blOn=true,isBusy=false,drawing=false,lastX=0,lastY=0,tt;
const spdLabels=['','slow','normal','fast'];
const viewKeys=['w','s','d',' ','h','n','t','u','b'];
...
const cvs=document.getElementById('cvs'),ctx=cvs.getContext('2d');let strokePts=[];
function getPos(e){const r=cvs.getBoundingClientRect(),sx=cvs.width/r.width,sy=cvs.height/r.height,s=e.touches?e.touches[0]:e;return{x:(s.clientX-r.left)*sx,y:(s.clientY-r.top)*sy};}
...
cvs.addEventListener('mousedown',startDraw);cvs.addEventListener('mousemove',moveDraw);cvs.addEventListener('mouseup',endDraw);cvs.addEventListener('mouseleave',endDraw);cvs.addEventListener('touchstart',startDraw,{passive:false});cvs.addEventListener('touchmove',moveDraw,{passive:false});cvs.addEventListener('touchend',endDraw);
...
void routeCmd() {
  ...
  switch (c) {
    ...
    case 'a':
      ...
      animLogoReveal();
      break;
    case 'h': currentView = VIEW_EYES_HEART;     animHeartEyes();     break;
    case 'n': currentView = VIEW_EYES_SLEEPY;    animSleepyEyes();    break;
    case 't': currentView = VIEW_EYES_STAR;      animStarEyes();      break;
    case 'u': currentView = VIEW_EYES_SURPRISE;  animSurprisedEyes(); break;
    case 'b': currentView = VIEW_EYES_BLUSH;     animBlushEyes();     break;
  }
}
...
void routeRedraw() {
  ...
  switch (currentView) {
    ...
    case VIEW_EYES_SLEEPY:   drawSleepyEyes();      break;
    case VIEW_EYES_STAR:     drawStarEyes();        break;
    case VIEW_EYES_SURPRISE: drawSurprisedEyes();   break;
